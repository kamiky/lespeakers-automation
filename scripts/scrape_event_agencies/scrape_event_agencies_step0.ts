 ...
 *    `--force`). Legacy monolithic JSON: skip city if every planned query exists.
...
async function runApifyGoogleMapsExtractor(params: {
  ...
  const { client, searchQueries, countryCode, language, maxResultsPerSearch } =
    params;
  ...
  const { items } = await client
    .dataset<ApifyPlace>(run.defaultDatasetId)
    .listItems();
...
async function main(): Promise<void> {
  ...
  const { cities: allCities, variants: allVariants } =
    loadCitiesAndVariants(country);
  ...
  if (step0Paths.length === 0 && !enrichedPath) {
    console.log(
      "[merge] No previous step0 / step1 / step2 output for this country, starting fresh.",
    );
  } else {
    console.log(
  ...
  for (const city of cities) {
    ...
    if (
      ...
      console.log(
        `[skip-city] "${city}" (${citySlug}) — already has data (pass --force to re-run).`,
      );
      continue;
    }
  ...
  console.log(
    `[cities] Will call Apify for ${citiesToScrape.length}/${cities.length} city(ies).`,
  );
  ...
  for (const city of citiesToScrape) {
    ...
    console.log(
      `[city] ---- ${city} (${citySlug}) — ${queries.length} search(es) ----`,
    );
  ...
  console.log(
    `[stats] Total agencies: ${total} (${withStep1} at step≥1 enrichments)`,
  );
  console.log("[done]");
}
